...
Sequel.migration do
  up do
    ...
    run 'CREATE INDEX IF NOT EXISTS idx_route_attempts_access_scope ON llm_route_attempts (access_scope)'
    run <<~SQL
      CREATE INDEX IF NOT EXISTS idx_route_attempts_identity_principal_id
      ON llm_route_attempts (identity_principal_id)
      WHERE identity_principal_id IS NOT NULL
    SQL
  end

  down do
    run 'DROP INDEX IF EXISTS idx_route_attempts_access_scope'
    run 'DROP INDEX IF EXISTS idx_route_attempts_identity_principal_id'
