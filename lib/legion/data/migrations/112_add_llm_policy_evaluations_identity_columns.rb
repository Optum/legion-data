...
Sequel.migration do
  up do
    ...
    run 'CREATE INDEX IF NOT EXISTS idx_policy_evaluations_access_scope ON llm_policy_evaluations (access_scope)'
    run <<~SQL
      CREATE INDEX IF NOT EXISTS idx_policy_evaluations_identity_principal_id
      ON llm_policy_evaluations (identity_principal_id)
      WHERE identity_principal_id IS NOT NULL
    SQL
  end

  down do
    run 'DROP INDEX IF EXISTS idx_policy_evaluations_access_scope'
    run 'DROP INDEX IF EXISTS idx_policy_evaluations_identity_principal_id'
