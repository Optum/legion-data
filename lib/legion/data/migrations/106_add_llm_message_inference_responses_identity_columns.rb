...
Sequel.migration do
  up do
    ...
    run 'CREATE INDEX IF NOT EXISTS idx_message_inference_responses_access_scope ON llm_message_inference_responses (access_scope)'
    run <<~SQL
      CREATE INDEX IF NOT EXISTS idx_message_inference_responses_identity_principal_id
      ON llm_message_inference_responses (identity_principal_id)
      WHERE identity_principal_id IS NOT NULL
    SQL
  end

  down do
    run 'DROP INDEX IF EXISTS idx_message_inference_responses_access_scope'
    run 'DROP INDEX IF EXISTS idx_message_inference_responses_identity_principal_id'
