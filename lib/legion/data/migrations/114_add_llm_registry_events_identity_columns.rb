...
Sequel.migration do
  up do
    ...
    run 'CREATE INDEX IF NOT EXISTS idx_registry_events_access_scope ON llm_registry_events (access_scope)'
    run <<~SQL
      CREATE INDEX IF NOT EXISTS idx_registry_events_identity_principal_id
      ON llm_registry_events (identity_principal_id)
      WHERE identity_principal_id IS NOT NULL
    SQL
  end

  down do
    run 'DROP INDEX IF EXISTS idx_registry_events_access_scope'
    run 'DROP INDEX IF EXISTS idx_registry_events_identity_principal_id'
